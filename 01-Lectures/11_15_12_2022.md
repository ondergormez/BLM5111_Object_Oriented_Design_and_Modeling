...
* Refactoring - Replace Type Code with Subclasses: 2 veya 3 sınıfa ayırabiliriz. 
  ...
  * Employe sınıfı içerisinde mühendis ve satış elemanı ayrımı yapılacak bir durum oluşmuyor olmalı.
...
Soru 3:  
Aşağıda soldaki UML şeması ile gösterilen sistemde hangi tasarım kalıbı kullanılmıştır? Açıklayınız.  
Soldaki tasarımda parça bütün ilişkisi var bu nedenle composite kalıbı kullanılmıştır diyebiliriz.

Soru 4:
Yukarıda sağdaki UML şeması ile gösterilen sistemde hangi tasarım kalıbı kullanılmıştır. Açıklayınız.  
